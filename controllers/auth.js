...
router.post('/register',  async (req, res) => {
  try {
    const { first_name, last_name, phone, address, number, email, role, } = req.body;
...
router.get('/admin/reset/:id',  async (req, res) => {
  try {
    const { id } = req.params;
    // Check if the username is already taken
...
router.post('/login', async (req, res) => {
  ...
  try {
      ...
      if (user.isLocked) {
          return res.status(401).json({ error: 'Account is locked. Please contact the admin' });
      }
      ...
      const token = jwt.sign(
          ...
          "al-Ilm-Foundation", // Use a secure secret key
          { expiresIn: '2h' } // Token expiration time
      );

      // Redirect based on user role using the isAdminMiddleware
      isAdminMiddleware(req, res, () => {
          // If the middleware allows, redirect to the admin dashboard
